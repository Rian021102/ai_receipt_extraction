...
def extract_json_from_response(response_text: str) -> str:
    ...
    # Fallback for model outputs that include prose around a JSON object.
...
def repair_json_with_model(bad_json_text: str) -> str:
    """
    Ask the model to repair malformed JSON while preserving values as much as possible.
    """
    if not bad_json_text:
        return ""
...
- Keep the same schema and values whenever possible.
- Fix escaping issues (for example unescaped double quotes inside strings).
- If a trailing partial object is cut off, remove only that broken fragment.
...
"""
    ...
    raw = ""
    if getattr(repair_response, "message", None) is not None:
        raw = repair_response.message.content or ""
...
"""
    ...
    for attempt in range(MAX_EXTRACT_RETRIES + 1):
        response = chat(
            ...
            options={
                "temperature": 0,
                # Keep generation deterministic per attempt but diversify retries.
                "seed": 42 + attempt,
                "num_ctx": 4096,
            },
        )
        ...
        try:
            ...
            # Fallback: ask the model to repair malformed JSON text.
            try:
                repaired_json_str = repair_json_with_model(json_str)
                if repaired_json_str:
                    last_json_str = repaired_json_str
                    last_data = json.loads(repaired_json_str)
    ...
    if last_data is None:
        preview_source = last_json_str or last_raw_content or ""
        preview = preview_source[:200].replace("\n", "\\n")
        parse_hint = f" last parser error: {last_parse_error}" if last_parse_error else ""
        raise json.JSONDecodeError(
            f"No valid JSON returned by model.{parse_hint} raw_len={len(last_raw_content)} preview='{preview}'",
            preview_source,
            0,
        )
